...
4. “文件”页支持按文件类型、科目和失效状态筛选。每个文件都可展开时间线，查看关联、取消关联、重新定位、改名、撤回及应用外内容变化；记录包含当时的路径、大小和修改时间。作业详情也会按“我的解答”“题目与参考”“其他文件”汇总该作业曾经关联过的文件历史，因此替换或取消关联旧解答后仍可追踪。在“设置”中导出或恢复 SQLite 备份，也可压缩数据库。备份包含记录、文件路径、文件历史、模块开关、自定义字段和设置，不包含实际文件、提示词文件、模块文件或 API Key。恢复后可在“文件”页重新定位失效路径。
5. 设置页可以删除已经添加的学期。删除前会显示课程和作业数量；确认后会级联移除该学期的课程、作业和关联记录，但不会删除磁盘上的学习文件。应用始终保留至少一个学期。

安装版和便携版使用同一数据库：`%APPDATA%\app.homeworkbook.local\homework.db`。移动便携版 EXE 不会移动数据库，已安装版和便携版在同一 Windows 账户下会看到同一数据。可编辑 AI 提示词位于 `%APPDATA%\app.homeworkbook.local\modules\assignment-import\prompts`，设置页可直接打开；修改后下次识别立即生效。升级时会从旧 `prompts` 目录迁移已有文件，仅更新未修改的旧默认提示词，手动改过的文件会保留。现有学习文件保持原位。课程目录搜索只在用户输入关键字时执行，不会在启动时批量扫描线性代数习题答案。
...
AI 作业导入的清单、Rust 实现、前端控件、服务商预设和提示词源文件都在 [modules/assignment-import](modules/assignment-import)；核心程序通过通用模块注册表加载入口。设置页把这一模块的开关、服务商、模型分配和提示词路径放在同一个卡片。关闭模块后入口会隐藏，核心模块宿主也会拒绝调用；配置与数据保持不变。外部模块放入 `%APPDATA%\app.homeworkbook.local\modules`，通过版本化 JSON 标准输入/输出协议调用；启动应用时不会执行外部模块。后续 PDF/DOCX 解析、AI 文件命名建议、打印等能力可由独立模块提供。清单格式、能力契约和安装方式见 [MODULES.md](MODULES.md)。模块只提出解析或命名结果；持久化和文件重命名由主程序负责。
...
前端通过 `src/service.ts` 的类型化接口访问核心 Tauri 命令；AI 模块的前端服务在 `modules/assignment-import/frontend/service.ts`。SQLite、文件和模块宿主分别在 `src-tauri/src/db.rs`、`files.rs`、`modules.rs`；模型请求在 `modules/assignment-import/backend.rs`。将来接入在线服务时，可替换服务实现并保留界面数据类型。
